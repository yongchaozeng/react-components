...
type TimeApiResponse = ApiResponse<number>
type CoffeeApiResponse = ApiResponse<unknown>
...
export function requestCurrentServerTime(callback: (response: TimeApiResponse) => void) {
  callback({
    status: 'success',
    data: Date.now()
  });
...
export function requestCoffeeMachineQueueLength(callback: (response: CoffeeApiResponse) => void) {
  callback({
    status: 'error',
